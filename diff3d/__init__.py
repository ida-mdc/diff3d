...
class SpatialLinearAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        q, k, v = rearrange_many(qkv, "b (h c) z x y -> b h c (z x y)", h=self.heads)
        ...
        out = rearrange(out, "b h c (z x y) -> b (h c) z x y", h=self.heads, z=f, x=h, y=w)
...
class Unet3D(nn.Module):
    def __init__(
        ...
        self.init_conv = nn.Conv3d(channels, init_dim, kernel_size=init_kernel_size, padding=init_padding)
